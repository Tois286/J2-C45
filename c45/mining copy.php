...
if (isset($_GET['table'])) {
    ...
    try {
        ...
        function calculateEntropy($count_good, $count_total)
        {
            ...
            $entropy = 0;
            if ($prob_good > 0) {
                $entropy -= $prob_good * log($prob_good, 2);
            }
            if ($prob_bad > 0) {
                $entropy -= $prob_bad * log($prob_bad, 2);
            }
        ...
        function buildDecisionTree($data, $best_attribute, $depth = 0, $max_depth = 10)
        {
            ...
            if ($depth >= $max_depth) {
                return getMajorityLabel($data); // Atau gunakan prediksi mayoritas
            }

            // Cek jika semua data dalam node saat ini memiliki label target yang sama
            $labels = array_column($data, 'KETERANGAN');
            if (count(array_unique($labels)) === 1) {
                return $labels[0]; // Semua data memiliki label yang sama
            }
            ...
            foreach ($groups as $value => $group) {
                ...
                if ($next_best_attribute !== null) {
                    ...
                    $tree[$best_attribute][$value] = getMajorityLabel($group); // Atau gunakan prediksi mayoritas
                }
            }
        ...
        // Fungsi untuk mendapatkan prediksi mayoritas dari data
